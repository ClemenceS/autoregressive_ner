...
def make_prompts(train_dataset, test_dataset, ner_tag, ner_tag_id, language, domain, begin_tag, end_tag, n_few_shots, criterion, prompt_keywords):
    ...
    pos_examples = random.sample([e for e in train_dataset if ner_tag_id in e['ner_tags']], 5)
    ...
    neg_examples = random.sample([e for e in train_dataset if set(e['ner_tags'])-{0,ner_tag_id}], 3)
