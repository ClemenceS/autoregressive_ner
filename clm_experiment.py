...
if project_root_nlstruct not in sys.path:
    sys.path.insert(0, project_root_nlstruct)
...
from nlstruct.data_utils import sentencize, slice_document
...
def line_chunk_sentencize(
        doc, lines_per_chunk=4, only_text=False, entity_overlap="raise", main_fragment_label=None):
        """
        Splits a document into chunks of N lines each,
        """
        text = doc["text"]
        lines = text.splitlines(keepends=True)  # keep line breaks
        ...
        for i in range(0, len(lines), lines_per_chunk):
            ...
            sentences.append(
                slice_document(
                    ...
                    only_text=only_text,
                    main_fragment_label=main_fragment_label)
            )
            begin = end
...
if __name__ == '__main__':
    ...
    args.add_argument("--dataset_type", type=str, help="if the dataset is article or code", default="article")
    ...
    if not args.transformers:
        llm = LLM(args.model_name, tensor_parallel_size=args.n_gpus, seed=args.random_seed, dtype="auto", gpu_memory_utilization=0.8)
